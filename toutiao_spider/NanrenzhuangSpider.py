...
@description:这是下载`http://enrz.com/`上的信息的程序
"""
import traceback
...
class NanrenzhuangSpider(BaseSpider):
    def __init__(self,config):
        super(NanrenzhuangSpider,self).__init__(config)
        self.url = 'http://enrz.com/'
        self.config = config
        self.name = 'NanrenzhuangSpider'
    ...
    def download_navigation(self,name,url,kw=None):
        ...
        try:
            ...
            result = []
            result.append({'url':'http://enrz.com/beauty','column':[u'美女']})
            result.append({'url':'http://enrz.com/info','column':[u'资讯']})
            result.append({'url':'http://enrz.com/artificiality','column':[u'恋物癖']})
            result.append({'url':'http://enrz.com/fashion','column':[u'时装']})
            result.append({'url':'http://enrz.com/guys','column':[u'男性']})
            result.append({'url':'http://enrz.com/special','column':[u'专题']})
    ...
    def __download_detail_item__(self,name,url,kw=None):
        ...
        try:
            ...
            read_txt = soup.find('div',attrs={'id':'content'})
            read_txt.find('h2').decompose()
            read_txt.find('h3').decompose()
    ...
    def __download_detail__(self,name,url,kw=None):
        ...
        try:
            ...
            tmp = div.find('h3').get_text().strip()
            source = tmp[tmp.find(u'编辑：')+len(u'编辑：'):tmp.find(u'发布时间：')].strip()
            ptime = tmp[tmp.find(u'发布时间：')+len(u'发布时间：'):tmp.find(u'星期')].strip()
    ...
    def download_list(self,name,url,kw=None):
        ...
        try:
            data = self.spider.get_data(url)
            pattern = re.compile(unicode('http://enrz.com/[a-z]+/\d{4}/\d{2}/\d{2}/\d+.html'))
            ...
            soup = BeautifulSoup(data,'lxml')
            tmp = soup.find('div',attrs={'class':'pagination'})
            next_url = None
            for a in tmp.find_all('a'):
                if a.get_text().find(u'下一页') >=0:
    ...
    def is_implemented(self):
        ...
        是否实现了该接口
        """
        return ProtoSpider.implementedBy(NanrenzhuangSpider)
...
if __name__ == '__main__':
    ...
    mn = NanrenzhuangSpider(config)
    result = mn.__download_detail__(mn.name,'http://enrz.com/fhm/2016/09/22/65706.html',{'column':[u'首页']})
    print result['image']
