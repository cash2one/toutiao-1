...
@description:这是下载`http://www.waduanzi.com/latest`上的信息的程序
"""
import traceback
...
class WaduanziSpider(BaseSpider):
    def __init__(self,config):
        super(WaduanziSpider,self).__init__(config)
        self.url = 'http://www.waduanzi.com/latest'
        self.config = config
        self.name = 'WaduanziSpider'
    ...
    def download_navigation(self,name,url,kw=None):
        ...
        try:
            result = []
            result.append({'url':'http://www.waduanzi.com/latest','column':[u'段子']})
    ...
    def __download_detail_item__(self,name,url,kw=None):
        ...
        try:
            ...
            read_txt = soup.find('div',attrs={'class':'item-content'})
            for div in read_txt.find('div'):
    ...
    def __download_detail__(self,name,url,kw=None):
        ...
        try:
            data = self.spider.get_data(url)
            soup = BeautifulSoup(data,'lxml')
            title  = soup.find('div',attrs={'class':'item-title'}).get_text()
            source = u'挖段子'
            tags = []
            result = {'name':self.name,'text':u'','title':title,'crawl_time':int(time.time()),
            'image':{},'source':source,'time':'','introduction ':'','tags':tags}
            print url
    ...
    def download_list(self,name,url,kw=None):
        ...
        try:
            data = self.spider.get_data(url)
            pattern = re.compile(unicode('http://www.waduanzi.com/archives/\d+'))
            ...
            soup =BeautifulSoup(data,'lxml')
            div = soup.find('div',attrs={'class':'cd-pages'})
            href = div.find('li',attrs={'class':'next'}).find('a').attrs['href']
    ...
    def is_implemented(self):
        ...
        是否实现了该接口
        """
        return ProtoSpider.implementedBy(WaduanziSpider)
...
if __name__ == '__main__':
    ...
    mn = WaduanziSpider(config)
    result = mn.__download_detail__(mn.name,'http://www.waduanzi.com/archives/195426',{'column':[u'首页']})
    print result
