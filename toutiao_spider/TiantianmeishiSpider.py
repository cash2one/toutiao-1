...
@description:这是下载`http://www.ttmeishi.com/CaiXi/JiaChangCai/`上的信息的程序
"""
import traceback
...
class TiantianmeishiSpider(BaseSpider):
    def __init__(self,config):
        super(TiantianmeishiSpider,self).__init__(config)
        self.url = 'http://www.ttmeishi.com/CaiXi/JiaChangCai/'
        self.config = config
        self.name = 'TiantianmeishiSpider'
    ...
    def download_navigation(self,name,url,kw=None):
        ...
        if name != self.name:
            ...
            return {}
        try:
            return {'name':name,'list':[{'url':'http://www.ttmeishi.com/CaiXi/JiaChangCai/','column':[u'美食']}]}
        except:
            traceback.print_exc()
            return {}
    ...
    def __download_detail_item__(self,name,url,kw=None):
        ...
        try:
            ...
            read_txt.find('h1').decompose()
            read_txt.find('div',attrs={'class':'cbox c_date'}).decompose()
            read_txt.find('div',attrs={'class':'c_fenxiang2 cbox'}).decompose()
            read_txt.find('div',attrs={'class':'c_shicai_xg cbox'}).decompose()
            read_txt.find('div',attrs={'class':'c_pinglun cbox'}).decompose()
    ...
    def __download_detail__(self,name,url,kw=None):
        ...
        if name != self.name:
            ...
            return {}
        try:
            data = self.spider.get_data(url,'gbk',retries=4)
            soup = BeautifulSoup(data,'lxml')
    ...
    def download_list(self,name,url,kw=None):
        ...
        try:
            details = []
            data = self.spider.get_data(url,'gbk')
            pattern = re.compile(unicode('/CaiPu/[a-z0-9]+.htm'))
            details = map(lambda x:{'url':urlparse.urljoin(url,x)},pattern.findall(data))
            
            soup = BeautifulSoup(data,'lxml')
            div = soup.find('div',attrs={'class':'content'}).find('div',attrs={'class':'pageNum'})
    ...
    def is_implemented(self):
        ...
        是否实现了该接口
        """
        return ProtoSpider.implementedBy(TiantianmeishiSpider)
...
if __name__ == '__main__':
    ...
    mn = TiantianmeishiSpider(config)
    result = mn.__download_detail__(mn.name,'http://www.ttmeishi.com/CaiPu/fd6812b1cdf9dfe5.htm')
    print result