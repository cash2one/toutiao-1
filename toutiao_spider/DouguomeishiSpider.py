...
@description:这是下载`http://www.douguo.com/caipu/zuixin/`上的信息的程序
"""
import traceback
...
class DouguomeishiSpider(BaseSpider):
    def __init__(self,config):
        super(DouguomeishiSpider,self).__init__(config)
        self.url = 'http://www.douguo.com/caipu/zuixin/'
        self.config = config
        self.name = 'DouguomeishiSpider'
    ...
    def download_navigation(self,name,url,kw=None):
        ...
        if name != self.name:
            ...
            return {}
        try:
            return {'name':name,'list':[{'url':'http://www.douguo.com/caipu/zuixin/','column':[u'美食']}]}
        except:
            traceback.print_exc()
            return {}
    ...
    def __download_detail_item__(self,name,url,kw=None):
        ...
        try:
            ...
            read_txt = soup.find('div',attrs={'class':'retew r3 pb25 mb20'})
            
            div = soup.find('div',attrs={'class':'bokpic'})
            read_txt = u'<div class="tmp">'+div.prettify()+u'\n'+read_txt.prettify()+'</div>'
            soup = BeautifulSoup(read_txt,'lxml')
            read_txt = soup.find('div',attrs={'class':'tmp'})
            
            read_txt.find('div',attrs={'class':'auth'}).decompose()
    ...
    def __download_detail__(self,name,url,kw=None):
        ...
        try:
            data = self.spider.get_data(url)
            soup = BeautifulSoup(data,'lxml')
            div = soup.find('div',attrs={'class':'recinfo'})
            title = div.find('h1',attrs={'id':'page_cm_id'}).get_text().strip()
            source = u'豆果美食'
    ...
    def download_list(self,name,url,kw=None):
        ...
        try:
            details = []
            data = self.spider.get_data(url)
            pattern = re.compile(unicode('http://www.douguo.com/cookbook/\d+.html'))
            details = map(lambda x:{'url':urlparse.urljoin(url,x)},list(set(pattern.findall(data))))
            
            soup = BeautifulSoup(data,'lxml')
            div = soup.find('div',attrs={'class':'pagination'})
            
            next_url = div.find('a',text=u'下一页')
            if next_url:
    ...
    def is_implemented(self):
        ...
        是否实现了该接口
        """
        return ProtoSpider.implementedBy(DouguomeishiSpider)
...
if __name__ == '__main__':
    ...
    mn = DouguomeishiSpider(config)
    result = mn.__download_detail__(mn.name,'http://www.douguo.com/cookbook/34.html')
    print result
